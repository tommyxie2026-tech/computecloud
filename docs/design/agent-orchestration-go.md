...
- 项目：computecloud
- 日期：2026-09-22
- 状态：v0.1 已实现轻量批任务子集；真实 CLI/独立双机未验收，扩展章节仍为目标设计
- 实现语言：Go
...
| 后续指令与恢复 | 目标为明确 session_ref 创建新 Task 并串行；v0.1 暂未开放 | 可保持会话热驻留 |
...
本节配置与域名为完整目标设计示例；v0.1 可用字段和命令以 examples 与运行指南为准。编码时固定 CLI 版本/校验值、证书、允许策略和凭据引用；若启用容器，再固定镜像摘要。
