...
日期：2026-09-22。状态：设计完成，未实现。配套[主设计](../design/gateway-mapreduce-v0.2.md)和[实施计划](../implementation/v0.2-plan.md)。

本文件定义 computecloud 自有 Job 接口；`/v1/jobs` 和下列 MCP 工具不是 Codex 原生方法。当前可用接口仍以 [runtime.proto](../../api/agent/v1/runtime.proto) 为准。请求 JSON Schema 和可解析示例位于 [v0.2 示例目录](../examples/v0.2/README.md)。
...
| `GET /v1/jobs/{id}/result` | `jobs:read` | 200 返回不可变终态结果；未结束返回 409 `JOB_NOT_FINISHED` |
...
`counts` 按 stage 和 Task state 聚合当前数据库事实；不把完成百分比伪装成准确剩余时间。`usage.coverage` 为 `complete`、`partial` 或 `unavailable`，未计量总数为 null。核心没有完整计量时不得返回 0 token。

终态结果含 `state`、`error`、`final_artifacts`、`child_failures`、`base_commit`、`manifest_sha256`、`usage`。失败/取消也可下载已获准的诊断产物，但 `final_artifacts` 不冒充成功报告。大报告通过下载引用提供，MCP 不把 tar 包内嵌进文本上下文。
...
| 429 | `ADMISSION_LIMIT` | 接收速率/排队上限，不等于执行失败 |
...
HTTP 路径 `/mcp`，采用 2025-11-25 Streamable HTTP 基线；固定 SDK/客户端版本验收后锁定。支持 `initialize`、`notifications/initialized`、`ping`、`tools/list`、`tools/call`；不提供 sampling、模型请求反向回调或实验性任务扩展。
...
模板键为 `[runtime_profile, policy_ref, acceptance_profile]` 的规范 JSON 数组之 SHA-256。模板内容摘要覆盖固定 CLI 版本、适配器模板版本、实际权限策略、验收命令 argv 和对应 Job 产物规则；排除秘密及机器绝对路径。双方使用同一版本编码函数：对象键排序、字符串原样保留、命令/参数顺序保留。Server 从操作者部署的受信模板清单取得期望值，不从 Worker 自报值推定信任。更改模板内容必须更新引用版本和摘要。
...
`findings.json` 最小结构：schema_version=`findings.v1`、base_commit、partition_key、findings 数组。每条 finding 包含 id、severity（info/low/medium/high/critical）、summary、path、line_start、line_end、evidence；路径/行号按基线检查，可选引用其他分片为 sources 数组。空 findings 合法；解析失败不等于“没有问题”。
...
报告 Reduce 的最终 JSON 使用 schema_version=`merged-report.v1`，包含 base_commit、manifest_sha256、summary、findings。每条 finding 保留上述字段，额外带 sources 数组（partition_key、finding_id）；新增发现使用 origin=`reduce` 且 sources 可为空，引用 Map 的发现使用 origin=`map` 且 sources 非空。Worker 校验引用确实存在于冻结输入，生成 review.json 与 review.md。行号从 1 起，line_end 不小于 line_start；所有代码证据必须能定位到基线。无法证实的结论在 summary 中注明，不伪造来源。
...
以下 SQL 是设计与约束验证用 DDL，不是已经接入程序的生产迁移。迁移入口必须从 v0.1 的 `store.Open` 改为逐版本执行；不能继续每次启动直接覆盖 user_version。
...
G1 使用至少 30 秒连接/响应头预算、300 秒流空闲预算、最长 30 分钟请求预算，并受绑定 Attempt 剩余期限约束；实现时暴露为配置。持续流转发不受普通短 HTTP WriteTimeout 意外截断，使用单次写入期限和整体 context；SSE 按事件完整性转发，不能用默认 Scanner 小缓冲截断大工具参数。
...
Attempt 模型 Token 至少 256 位随机，原文通过受保护的 Assignment 交付，表内只存 SHA-256。其 owner/project/job/model 由当前 Attempt 及冻结规格推导，不信任客户端的追踪头。普通模型调用方只能使用自己身份对应的固定项目；不能伪造 job_id 归属。
...
- 核心 Job API、MCP、G1 Responses 各自独立做契约测试；示例标为设计用，v0.1 配置解析器会拒绝尚不存在的键。
