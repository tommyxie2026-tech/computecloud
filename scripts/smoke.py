    ...
    with tempfile.TemporaryDirectory(prefix="computecloud-smoke-") as temporary:
        ...
        try:
            ...
            server_config = write("server.json", {"server": {
                ...
                "users": [{"token_file": user_token, "owner": "operator", "projects": ["demo"], "credentials": ["fixture-account"]}], "workers": workers}})
            write("client.json", {"client": {"address": address, "token_file": user_token, "tls": tls}})
            ...
            with sqlite3.connect(restored / "state.db") as database:
                ...
                assert database.execute("SELECT count(*) FROM tasks").fetchone()[0] == 7
